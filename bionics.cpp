...
// Header text
 mvwprintz(w, 0,  0, c_white, _("Installing bionics:"));
...
// Helper text
 mvwprintz(w, 2, 40, c_white,        _("Difficulty of this module: %d"),
           type->difficulty);
 mvwprintz(w, 3, 40, c_white,        _("Your installation skill:   %d.%d"),
           skint, skdec);
 mvwprintz(w, 4, 40, c_white,       _("Installation requires high intelligence,"));
 mvwprintz(w, 5, 40, c_white,       _("and skill in electronics, first aid, and"));
 mvwprintz(w, 6, 40, c_white,       _("mechanics (in that order of importance)."));

 int chance_of_success = int((100 * pl_skill) /
                             (pl_skill + 4 * type->difficulty));

 mvwprintz(w, 8, 40, c_white,        _("Chance of success:"));
 ...
 mvwprintz(w, 10, 40, c_white,       _("Failure may result in crippling damage,"));
 mvwprintz(w, 11, 40, c_white,       _("loss of existing bionics, genetic damage"));
 mvwprintz(w, 12, 40, c_white,       _("or faulty installation."));
 wrefresh(w);

 if (type->id == itm_bionics_battery) {	// No selection list; just confirm
  mvwprintz(w,  2, 0, h_ltblue, _("Battery Level +%d"), BATTERY_AMOUNT);
  mvwprintz(w, 22, 0, c_ltblue, _("\
