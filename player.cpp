...
// Print name and header
 mvwprintw(w_grid, 0, 0, "%s - %s", name.c_str(), (male ? _("Male") : _("Female")));
 mvwprintz(w_grid, 0, 39, c_ltred, _("| Press TAB to cycle, ESC or q to return."));
// Main line grid
 for (int i = 0; i < 80; i++) {
...
// Next, draw traits.
 ...
 mvwprintz(w_traits, 0, 9, c_ltgray, _("TRAITS"));
 for (int i = 0; i < PF_MAX2; i++) {
  if (my_traits[i]) {
...
detecting traps and other things of interest."));
  ...
  case 2:	// Encumberment tab
   mvwprintz(w_encumb, 0, 0, h_ltgray, _("      ENCUMBERANCE        "));
   if (line == 0) {
    mvwprintz(w_encumb, 2, 2, h_ltgray, _("Head"));
    mvwprintz(w_info, 0, 0, c_magenta, _("\
Head encumberance has no effect; it simply limits how much you can put on."));
   } else if (line == 1) {
    mvwprintz(w_encumb, 3, 2, h_ltgray, _("Eyes"));
    mvwprintz(w_info, 0, 0, c_magenta, _("\
...
double(double(encumb(bp_eyes)) / 2));
    ...
    mvwprintz(w_encumb, 4, 2, h_ltgray, _("Mouth"));
    mvwprintz(w_info, 0, 0, c_magenta, _("\
Running costs +%d movement points"), encumb(bp_mouth) * 5);
   } else if (line == 3) {
    mvwprintz(w_encumb, 5, 2, h_ltgray, _("Torso"));
    mvwprintz(w_info, 0, 0, c_magenta, _("\
...
encumb(bp_torso) * (80 - sklevel[sk_swimming] * 3), encumb(bp_torso) * 20);
    ...
    mvwprintz(w_encumb, 6, 2, h_ltgray, _("Hands"));
    mvwprintz(w_info, 0, 0, c_magenta, _("\
Reloading costs +%d movement points;\n\
Dexterity -%d when throwing items"), encumb(bp_hands) * 30, encumb(bp_hands));
   } else if (line == 5) {
    mvwprintz(w_encumb, 7, 2, h_ltgray, _("Legs"));
...
Dodge skill %s%.1f"), sign.c_str(), encumb(bp_legs) * 3,
    ...
    mvwprintz(w_encumb, 8, 2, h_ltgray, _("Feet"));
    mvwprintz(w_info, 0, 0, c_magenta, _("\
...
void player::disp_morale()
{
 ...
 mvwprintz(w, 2, 20, c_ltgray, _("Value"));
...
{
 mvwprintz(w, 1, 0, c_ltgray, _("Weapon: %s"), weapname().c_str());
 if (weapon.is_gun()) {
       if (recoil >= 36)
   mvwprintz(w, 1, 30, c_red,    _("Recoil"));
  else if (recoil >= 20)
   mvwprintz(w, 1, 30, c_ltred,  _("Recoil"));
  else if (recoil >= 4)
   mvwprintz(w, 1, 30, c_yellow, _("Recoil"));
  else if (recoil > 0)
   mvwprintz(w, 1, 30, c_ltgray, _("Recoil"));
 }
 ...
 mvwprintz(w, 2, 45, col_xp, "%d", xp_pool);
      ...
      if (pain - pkill >= 50)
  mvwprintz(w, 3, 0, c_red,    _("Excrutiating pain!"));
 else if (pain - pkill >= 40)
  mvwprintz(w, 3, 0, c_ltred,  _("Extreme pain"));
 else if (pain - pkill >= 30)
  mvwprintz(w, 3, 0, c_ltred,  _("Intense pain"));
 else if (pain - pkill >= 20)
  mvwprintz(w, 3, 0, c_yellow, _("Heavy pain"));
 else if (pain - pkill >= 10)
  mvwprintz(w, 3, 0, c_yellow, _("Moderate pain"));
 else if (pain - pkill >   0)
  mvwprintz(w, 3, 0, c_yellow, _("Minor pain"));
 ...
 mvwprintz(w, 3, 20, col_str, _("Str %s%d"), str_cur >= 10 ? "" : " ", str_cur);
 mvwprintz(w, 3, 27, col_dex, _("Dex %s%d"), dex_cur >= 10 ? "" : " ", dex_cur);
 mvwprintz(w, 3, 34, col_int, _("Int %s%d"), int_cur >= 10 ? "" : " ", int_cur);
 mvwprintz(w, 3, 41, col_per, _("Per %s%d"), per_cur >= 10 ? "" : " ", per_cur);
 mvwprintz(w, 3, 48, col_spd, _("Spd %s%d"), spd_cur >= 10 ? "" : " ", spd_cur);
...
void player::power_bionics(game *g)
{
 ...
 mvwprintz(wBio, 0, 0, c_blue, _("BIONICS -"));
 mvwprintz(wBio, 0,10, c_white,
           _("Activating.  Press '!' to examine your implants."));
 ...
 nc_color type;
 if (passive.size() > 0) {
  mvwprintz(wBio, 2, 0, c_ltblue, _("Passive:"));
 ...
 if (active.size() > 0) {
  mvwprintz(wBio, 2, 32, c_ltblue, _("Active:"));
 ...
 do {
  ...
  if (ch == '!') {
   ...
   if (activating)
    ...
    mvwprintz(wBio, 0, 10, c_white,
              _("Examining.  Press '!' to activate your implants."));
  } else if (ch == ' ')
   ch = KEY_ESCAPE;
...
int player::pick_usb()
{
 ...
 int select = menu_vec(_("Choose drive:"), selections);
...
bool player::eat(game *g, int index)
{
 ...
 if (index == -2) {
  ...
  if (weapon.is_food_container(this)) {
   ...
   if (!is_npc())
    g->add_msg(_("You can't eat your %s."), weapon.tname(g).c_str());
   else
  ...
  if (inv[index].is_food_container(this)) {
   ...
   if (!is_npc())
    g->add_msg(_("You can't eat your %s."), inv[index].tname(g).c_str());
   else
...
// Remember, comest points to the it_comest data
  ...
  if (comest->tool != itm_null) {
   ...
   if (!has) {
    if (!is_npc())
     g->add_msg(_("You need a %s to consume that!"),
  ...
  if (has_trait(PF_CARNIVORE) && eaten->made_of(VEGGY) && comest->nutr > 0) {
   if (!is_npc())
    g->add_msg(_("You can only eat meat!"));
   else
    g->add_msg(_("Carnivore %s tried to eat meat!"), name.c_str());
   return false;
  }

  if (has_trait(PF_VEGETARIAN) && eaten->made_of(FLESH) && !is_npc() &&
      !query_yn(_("Really eat that meat? (The poor animals!)")))
   return false;
...
// Descriptive text
  if (!is_npc()) {
   if (eaten->made_of(LIQUID))
    g->add_msg(_("You drink your %s."), eaten->tname(g).c_str());
   else if (comest->nutr >= 5)
    g->add_msg(_("You eat your %s."), eaten->tname(g).c_str());
  ...
  if (has_trait(PF_VEGETARIAN) && eaten->made_of(FLESH)) {
   if (!is_npc())
    g->add_msg(_("You feel bad about eating this meat..."));
 ...
 if (eaten->charges <= 0) {
  if (which == -1)
   ...
   if (!is_npc())
    g->add_msg(_("You are now wielding an empty %s."), weapon.tname(g).c_str());
   ...
   if (!is_npc())
    g->add_msg(_("%c - an empty %s"), inv[which].invlet,
...
bool player::wield(game *g, int index)
{
 if (weapon.type->id == itm_bio_claws) {
  g->add_msg(_("You cannot unwield your claws!  Withdraw them with 'p'."));
  return false;
 }
 if (index == -3) {
  if (!is_armed()) {
...
// Make sure we're not wearing 2 of the item already
 ...
 if (count == 2) {
  g->add_msg(_("You can't wear more than two %s at once."),
             to_wear->tname().c_str());
  return false;
 }
 ...
 if (armor->covers & mfb(bp_head) && encumb(bp_head) != 0) {
  g->add_msg(_("You can't wear a%s helmet!"),
             wearing_something_on(bp_head) ? _("nother") : "");
  return false;
 }
 ...
 if (armor->covers & mfb(bp_head) && !to_wear->made_of(WOOL) &&
  ...
  g->add_msg(_("You cannot wear a helmet over your %s."),
             (has_trait(PF_HORNS_POINTED) ? _("horns") :
              (has_trait(PF_ANTENNAE) ? _("antennae") : _("antlers"))));
  return false;
 }
 ...
 for (body_part i = bp_head; i < num_bp; i = body_part(i + 1)) {
  if (armor->covers & mfb(i) && encumb(i) >= 4)
   g->add_msg(_("Your %s %s very encumbered! %s"),
              body_part_name(body_part(i), 2).c_str(),
              (i == bp_head || i == bp_torso || i == bp_mouth ? _("is") : _("are")),
...
// Check if reading is okay
 if (g->light_level() < 8) {
  g->add_msg(_("It's too dark to read!"));
  return;
 }
...
// Some macguffins can be read, but they aren't treated like books.
 ...
 if (tmp->intel > 0 && has_trait(PF_ILLITERATE)) {
  ...
  g->add_msg(_("This book is way too complex for you to understand."));
  return;
...
// Kevlar, plastic, iron, steel, and silver may be damaged by BASHING damage
   ...
   if (worn[i].damage >= 5) {
    int linet;
    if (!is_npc())
     g->add_msg(_("Your %s is completely destroyed!"), worn[i].tname(g).c_str());
    else if (g->u_see(posx, posy, linet))
     g->add_msg(_("%s's %s is destroyed!"), name.c_str(),
                worn[i].tname(g).c_str());
    worn.erase(worn.begin() + i);
   }
  }
