...
void addict_effect(game *g, addiction &add)
{
 ...
 case ADD_CIG:
  if (in > 20 || one_in((500 - 20 * in))) {
   g->add_msg(_("You %s a cigarette."),
              rng(0, 6) < in ? _("need") : _("could use"));
...
std::string addiction_name(addiction cur)
{
 switch (cur.type) {
  ...
  default:		return _("Erroneous addiction");
 }
}
...
Risk of delirium tremens");
 ...
 case ADD_PKILLER:
  dump << _("Strength -") << strpen << _(";   Perception - 1;   Dexterity - 1;") <<
          std::endl <<
  _("Depression and physical pain to some degree.  Frequent cravings.  Vomiting.");
  return dump.str();
